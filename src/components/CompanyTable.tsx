...
export function CompanyTable({ companies, onVerDetalhes, onAgendar }: CompanyTableProps) {
  ...
  return (
    <div>
      ...
      <div className="max-h-[560px] overflow-auto rounded-2xl border border-[var(--color-border)] shadow-[var(--shadow-sm)]">
        <table className="w-full min-w-[600px] bg-[var(--color-bg-secondary)] text-sm">
          <thead className="sticky top-0 z-10 bg-[var(--color-bg-tertiary)]/95 text-left text-xs uppercase tracking-wide font-semibold text-[var(--color-text-secondary)] shadow-[0_1px_0_var(--color-border)] backdrop-blur">
            <tr>
              ...
              {COLUNAS.map((col) => (
                <th key={col.key} className={`px-4 py-3 ${col.ocultaMobile ? 'hidden lg:table-cell' : ''}`}>
                  <button
                    className="inline-flex items-center gap-1 font-semibold transition-colors hover:text-[var(--color-primary-ink)]"
          ...
          <tbody className="divide-y divide-[var(--color-border-light)]">
            {visiveis.map((company) => (
              <tr key={company.id} className="transition-colors duration-150 hover:bg-[var(--color-bg-tertiary)]">
                ...
                <td className="w-[220px] max-w-[220px] px-4 py-3 align-top">
                  <button
                    ...
                    className="block w-full break-words text-left font-medium leading-snug text-[var(--color-primary)] hover:text-[var(--color-primary-dark)] transition-colors"
                  >
                    {company.razaoSocial}
                  </button>
                </td>
                <td className="hidden px-4 py-3 align-top lg:table-cell">{company.cidade}</td>
                <td className="px-4 py-3 align-top">{setorCurto(company.setor)}</td>
                ...
                <td className="px-4 py-3 align-top">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="h-1.5 w-14 overflow-hidden rounded-full bg-[var(--color-bg-tertiary)]">
                      <div
                ...
                <td className="px-4 py-3 align-top">
                  <div className="flex gap-1">
                    <button
                      ...
                      className="rounded p-1.5 text-[var(--color-text-tertiary)] transition-colors hover:bg-[var(--color-primary-light)] hover:text-[var(--color-primary-ink)]"
                    >
                    ...
                    <button
                      ...
                      className="rounded p-1.5 text-[var(--color-text-tertiary)] transition-colors hover:bg-[var(--color-bg-tertiary)] hover:text-[var(--color-secondary)]"
                    >
