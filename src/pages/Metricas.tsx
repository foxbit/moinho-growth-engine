      ...
      <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
        {KPIS.map(({ chave, rotulo, variacao }) => (
          <Card key={chave}>
            ...
            <p className="mt-1 text-3xl font-bold tabular-nums">{metricas[chave]}</p>
            <p
              className={`mt-2 inline-flex items-center gap-1 text-xs font-medium tabular-nums ${
                variacao >= 0 ? 'text-[var(--color-secondary)]' : 'text-[var(--color-critical-ink)]'
              }`}
            >
          ...
          {funil.map((etapa) => {
            ...
            return (
              <div key={etapa.rotulo}>
                ...
                <div className="h-5 w-full overflow-hidden rounded bg-[var(--color-bg-tertiary)]">
                  <div
                    className={`h-full rounded ${etapa.cor} transition-[width] duration-500`}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Card title="Ocupação do Moinho">
          <div className="flex items-center gap-6">
            ...
            >
              <div className="flex h-24 w-24 items-center justify-center rounded-full bg-[var(--color-bg-secondary)] text-2xl font-bold tabular-nums">
      ...
      <Card>
        <dl className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2 xl:grid-cols-4">
          <div>
            ...
            <dd className="text-xl font-bold tabular-nums">{formatCurrency(metricas.receita)}</dd>
          </div>
          <div>
            <dt className="text-[var(--color-text-secondary)]">Ciclo de Vendas Médio</dt>
            <dd className="text-xl font-bold tabular-nums">{metricas.cicloVendasMedio} dias</dd>
          </div>
          <div>
            <dt className="text-[var(--color-text-secondary)]">Conversão Outbound</dt>
            <dd className="text-xl font-bold tabular-nums">{metricas.taxaConversaoOutbound}%</dd>
          </div>
          <div>
            <dt className="text-[var(--color-text-secondary)]">Conversão Eventos</dt>
            <dd className="text-xl font-bold tabular-nums">{metricas.taxaConversaoEventos}%</dd>
          </div>
        </dl>
      </Card>
    </div>
